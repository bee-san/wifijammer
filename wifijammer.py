...
logging.debug("Imports sucessful")
...
def parse_args():
	logging.debug("Setting up arguments")
	...
	parser.add_argument("-v", help="Gives verbose output", action="store_true")
...
def get_mon_iface(args):
	logging.debug("checking to see if monitor mode exists")
	# checks to see if a monitor mode interface alreadt exists
	# creates a global variable called monitor_on
	global monitor_on
...
def iwconfig():
	logging.info("performing iwconfig")
	# use the subproces library to call iwconfig and store it in a varaiblr
	# string manipulation to find just the interface names
	# for some reason, when iwconfig is performed as root it'll only print the wireless card
	# and not ethernet.... what the firetruck?
	monitors = []
	interfaces = {}
	try:
...
def get_iface(interfaces):
	logging.debug("getting the most powerful interface")
	# checks to see if any wirleess interfaces were found.
	scanned_aps = []
...
def start_mon_mode(interface):
	logging.debug("parsing ifconfigand transforming it into monitor mode")
	# starts monitor mode on the wireless card
...
def remove_mon_iface(mon_iface):
	logging.debug("getting monitor interface")
	# subprocess popen is slower than os.system, and we don't need to return the output to a variable
	# so os.system is being used here
	os.system('ifconfig %s down' % mon_iface)
	os.system('iwconfig %s mode managed' % mon_iface)
	os.system('ifconfig %s up' % mon_iface)

def mon_mac(mon_iface):
	logging.debug("parsing ifconfig")
	# some operating systems format ifconfig differently, this fixes it
...
def channel_hop(mon_iface, args):
	...
	while 1:
		...
		# prints the channel hops to screen
		if args.channel:
			time.sleep(.05)
		else:
			...
			if first_pass == 1:
				time.sleep(1)
				continue
				# waits a second so it can pick up other AP's on the same channel on first pass thru
		logging.debug("printed to screen, now DOSING")

		deauth(monchannel)


def deauth(monchannel):
	logging.debug("starting to deauth")
	# starts deauthign the channel the wireless receiver is on, becasue it takes time to change channels
	'''
	...
	# this sends deauth packets to AP's to broadcast, many AP's ignore broadcast's so the -d option
	# sends deauths directly to clients instead of the AP
	if len(APs) > 0:
...
def output(err, monchannel):
	logging.debug("outputting to screen")
	# outputs the monitor mode channel and all the targets in the APs and clients_APs
	# in a way to make it look pretty
	os.system('clear')
	#TODO I don't like the clear command, maybe use \n\n instead?
	if err:
		print(err)
	else:
...
def noise_filter(skip, addr1, addr2):
	...
	# ignores reserved MAC addresses
	if skip:
		ignore.append(skip)
	for i in ignore:
...
def cb(pkt):
	logging.debug("at cb(pkt), sniffing callback")
	# we have 2 threads, one for channel hopping and one for sniffing. this is the callback from sniffing,
	# we're just checing to see if any arguments were passed.
	'''
	...
	# The order here is very important. First we update the list of APs, then we check if the packet has an ignorable
	# addr1 or addr2, then we append new client/AP combos to the clients_APs list.
	#  You can’t check for ignorable MACs first because Beacon frames go to broadcast addresses.
...
def clients_APs_add(clients_APs, addr1, addr2):
	logging.debug("adds clients from AP's")
	if len(clients_APs) == 0:
		if len(APs) == 0:
			with lock:
...
def AP_check(addr1, addr2):
	# A quick function to check if the AP MAC in a client/AP MAC pair already exists in the APs list. If it does,
	# then we can use the channel and SSID from the matching AP in the APs list when appending that
	# client/AP pair to the clients_APs list.
	for ap in APs:
...
def stop(signal, frame):
	# when asked to stop, shuts off monitor mode.
	if monitor_on:
		sys.exit('\n['+R+'!'+W+'] Closing')
	else:
...
if __name__ == "__main__":
	...
	# this catches CRTL+C's and stops the program from closing immeditally, turns off monitor mode etc first
	try:
